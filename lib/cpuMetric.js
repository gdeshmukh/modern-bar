...
class CpuMetric extends St.BoxLayout {
    ...
    // reads — no subprocess. (A GPU metric is deliberately absent: it would
    // need a vendor tool spawned per poll, and only NVIDIA reports usefully.)
    _readExtras(done) {
        let pending = 2;
        const finish = () => {
            if (--pending === 0 && done)
    ...
    _sample() {
        ...
        file.load_contents_async(null, (f, res) => {
            // A read can complete after destroy(); rendering would then touch a
            // destroyed St.Label and log GJS criticals.
            if (this._destroyed)
                return;
            let contents;
            try {
    ...
    destroy() {
        // Set FIRST so any /proc read still in flight bails in its callback
        // instead of rendering into a destroyed actor.
