...
//     "keep the last value on show", never as an error dialog. A failure run
//     is still VISIBLE: once the reading is provably stale (or there is no
//     reading at all) the metric powers down — dimmed, with a small
//     connection-lost glyph — instead of lying with a bright number or
//     vanishing. Clicking it (opening the popup) is the manual refresh.
...
// How old a reading may be and still be shown at full brightness. Beyond an
// hour the 5-hour window has moved on enough that the number would mislead if
// it looked current — so past this line the metric renders POWERED DOWN
// (dimmed + connection-lost glyph), never bright, never hidden. The popup
// always states the true age either way.
...
class ClaudeMetric extends St.BoxLayout {
    ...
    _init(settings, iconsPath, popupGroup = null) {
        ...
        // Powered-down marker: only shown while the metric is degraded (stale
        // reading, or failing with nothing to show). Small and trailing so the
        // healthy layout doesn't move when it appears.
        ...
        // Recovery model, deliberately monitor-free: a file monitor on
        // ~/.claude/.credentials.json and a Gio.NetworkMonitor kick were both
        // tried here and REMOVED — a standing watcher on a sensitive path,
        // plus a signal that fires for docker bridges and VPN churn, is a lot
        // of machinery for shaving minutes off an automatic recovery. What
        // remains: the retry chain keeps cycling on its own (backoff, quiet
        // periods, revival — see _scheduleRetry/_fetch), the panel shows a
        // degraded state honestly instead of hiding (_syncState), and opening
        // the popup is a user-initiated forced fetch (_onPopupOpenChanged).
    ...
    // Popup open = the user's click, and the ONLY manual-refresh control. When
    // the metric is degraded (failing or stale) that click forces a fetch,
    // bypassing any quiet period on the same principle as _restart: an
    // explicit user action beats politeness. A HEALTHY metric's popup still
    // costs zero extra requests — hard requirement, the endpoint rate-limits —
    // so the render below reuses the response the panel number already has.
    // No buttons in the popup: the no-grab design holds, the click that opens
    // it IS the refresh. When the forced fetch lands while the popup is open,
    // _send re-renders it in place.
    ...
    _isStale() {
        if (!this._hasValue || !this._lastFetchOk)
            return false;
        return Math.floor(Date.now() / 1000) - this._lastFetchOk > CACHE_MAX_AGE;
    }

    // The single writer of the widget's outward state. A long failure run
    // (stale token, endpoint outage) once kept painting the last number as if
    // current — that is exactly how "100%" stayed bright on the panel for
    // hours after the 5-hour window had reset — and the first fix HID the
    // metric instead, which was honest but indistinguishable from broken.
    // Now the metric never lies in either direction:
    //
    //   fresh value            -> normal (palette rest, powers up on hover)
    //   value older than 1h    -> value stays, DIMMED + connection-lost glyph
    //   failing with no value  -> glyph only, dimmed — present and clickable,
    //                             so the popup-open manual refresh is reachable
    //   no value, not failing  -> hidden (startup, before the first attempt)
    //
    // Degradation deliberately does NOT re-eager the backoff: the panel is
    // showing an honest state, so automatic recovery can stay polite — the
    // user's click (forced fetch) is the fast path now. Alert coloring is
    // suppressed by the stale class in CSS: a dimmed 100% must not blaze.
    // Runs on every fetch attempt, so a value crossing the 1h line powers
    // down within one interval tick.
    _syncState() {
    ...
    _fetch(force = false) {
        // Runs on every attempt (interval tick, retry, forced), so a value
        // crossing the staleness line powers down within one refresh interval.
        this._syncState();
        ...
        if (!force && this._quietUntil && now < this._quietUntil) {
            // The chain can die here: _scheduleRetry extends _quietUntil while
            // an already-armed one-shot keeps its OLD fire time, so that retry
            // can land inside the hold, bail on this branch, and leave nothing
            // armed. Re-arm for the remainder so the backoff owns its own
            // recovery instead of leaning on the interval timer (which can be
            // configured as slow as an hour).
            if (!this._retryId) {
        ...
        this._session.send_and_read_async(msg, GLib.PRIORITY_DEFAULT, null, (sess, res) => {
            ...
            try {
                ...
                if (typeof util !== 'number') {
                    ...
                    // Back off long; a later poll reads the fresh token once
                    // Claude Code writes one, and clicking the metric forces a
                    // fetch immediately.
                ...
                // If the popup is watching (it may have triggered this very
                // fetch as a manual refresh), show the fresh numbers in place.
    ...
    _loadCache() {
        try {
            ...
            if (pct < 0 || !at)
                return;
            if (Math.floor(Date.now() / 1000) - at < 0)
                return;          // future timestamp — clock skew, distrust it
            // Seed WHATEVER age the cache is. An over-age value used to be
            // ignored here (stay hidden), but the lock screen destroys and
            // reconstructs the extension, so a metric honestly showing a
            // dimmed stale value would vanish on unlock — the exact
            // hidden-vs-honest inconsistency this branch removes. _syncState
            // (via _syncVisible at the end of _init) renders an old seed
            // powered-down, and the popup states its true age.
