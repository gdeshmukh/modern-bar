...
export class MetricPopup {
    ...
    //
    // `tail` optionally appends a SECOND, marked value after the first:
    //   {gicon, text}  ->  [icon] name … value [gicon] text
    // It exists because a bare trailing number is ambiguous — "78° / 57°  20%"
    // gives no clue that the last figure is a chance of rain. The marker has to
    // be an icon rather than a character: the popup is set in JetBrains Mono,
    // which has no ☂ or 💧, so a glyph would silently fall back to another face
    // (or to colour emoji) instead of tinting with the palette.
