...
//
// Shows how far through the configured work day we are, "WD nn%". Hidden
// entirely outside work hours (and when show-workday is off). One low-frequency
// timer — the value only needs to move about once a minute.
...
class WorkdayMetric extends St.BoxLayout {
    _init(settings) {
        super._init({
            ...
            reactive: true,   // enables CSS :hover feedback (cosmetic for now)
        });
    ...
    _update() {
        ...
        this._label.set_text(`WD ${Math.max(0, Math.min(100, pct))}%`);
        this.visible = true;
    }
