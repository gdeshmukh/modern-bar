...
// Exported so out-of-tree test harnesses can drive _evaluate() directly —
...
export class MenuWatch {
    ...
    // moving back up to it doesn't close), the dropdown itself, and any open QS
    // sub-menu, which is NOT inside the dropdown's actor.
    //
    // ⚠ `menu.actor` is NOT reliably the visible dropdown, and assuming it was
    // caused the live "it closes when I leave the top bar, not the dropdown"
    // bug. QuickSettingsMenu replaces `actor` with a deliberately ZERO-SIZED
    // wrapper whose child is the real boxpointer:
    //
    //     this.actor = new St.Widget({reactive: true, width: 0, height: 0});
    //     this.actor.add_child(this._boxPointer);
    //
    // while the calendar's `actor` IS its boxpointer. A 0x0 rect is discarded
    // as unmeasurable, so for Quick Settings the panel button was the only
    // thing left to measure — which is exactly the reported symptom.
    //
    // So: offer every plausible container and keep whatever actually measures.
    // Extra entries are free (rectOf drops anything unmeasurable, and the inner
    // boxes are contained by the outer ones), and this survives a shell that
    // shuffles which one is canonical.
    //
    ...
    _rects() {
        ...
        // QuickSettingsMenu tracks the sub-menu it has open. If a shell upgrade
        // renames it we simply lose sub-menu tolerance rather than breaking.
